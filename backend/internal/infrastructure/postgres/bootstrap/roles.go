...
// systemScopeID is the fixed scope for global organization role templates.
var systemScopeID = uuid.Nil

// SeedRoles ensures default RBAC role templates and permissions exist.
func SeedRoles(ctx context.Context, db *pgxpool.Pool) error {
	...
	for _, role := range roles {
		roleID, err := upsertRole(ctx, db, role.name, role.description)
		if err != nil {
			return err
		}
		...
		for _, perm := range role.permissions {
			...
			`, roleID, perm)
			if err != nil {
				return fmt.Errorf("insert permission %s for role %s: %w", perm, role.name, err)
			}
		}
	}

...
func NewHandler(
	...
	return &Handler{
		...
		deleteWorkspaceUC: deleteWorkspaceUC,
		orgRepo:           orgRepo,
		appRepo:           appRepo,
	}
}
...
func (h *Handler) ListOrgs(w http.ResponseWriter, r *http.Request) {
	orgs, err := h.listOrgsUC.Execute(r.Context())
	if err != nil {
		response.Error(w, err)
		return
	}

	response.JSON(w, http.StatusOK, pagination.NewResult(orgs, pagination.Params{
		Page:    1,
...
func (h *Handler) GetWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	workspace, err := h.getWorkspaceUC.Execute(r.Context(), workspaceID)
	if err != nil {
		response.Error(w, err)
		return
	}
...
// DeleteWorkspace archives a workspace.
func (h *Handler) DeleteWorkspace(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.deleteWorkspaceUC.Execute(r.Context(), workspaceID); err != nil {
		response.Error(w, err)
		return
	}

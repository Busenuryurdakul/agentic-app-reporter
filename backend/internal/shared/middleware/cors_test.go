...
func TestCORSOptions_AllowsCredentialsForExplicitOrigins(t *testing.T) {
	...
	assert.Contains(t, opts.AllowedHeaders, "X-Organization-ID")
	assert.Contains(t, opts.AllowedHeaders, "X-Workspace-ID")
	assert.Contains(t, opts.AllowedHeaders, "X-Workspace-Slug")
}
